...
def main():
    ...
    main_dict = torch.load("../save_model_ssda/resnet34_real_sketch_SEW_2000.ckpt.pth.tar")
    ...
    if not is_dict_saved:
        with torch.no_grad():
            ...
            for idx, image_obj in enumerate(source_loader):
                ...
                features_dict.name_list.append(image_obj[2])
                features_dict.gt_labels.append(image_obj[1].cpu().data.item())
                print(image_obj[1].cpu().data.item())
            ...
            for idx, image_obj in enumerate(target_loader_test):
                ...
                print(pred_label)
                print(gt_label)
                features_dict.is_correct_label.append(pred_label == gt_label)
                features_dict.name_list.append(image_obj[2])
        ...
        with open('features_dict_%s_%s.pickle'%(args.source,args.target), 'wb') as handle:
            pickle.dump(features_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)
    
    elif is_dict_saved:
        with open('features_dict_%s_%s.pickle'%(args.source,args.target), 'rb') as handle:
            features_dict = pickle.load(handle)
        

    
    features_to_plot = np.array(features_dict.feat_global)
    print("Shape of features to be plotted on tSNE is : ", features_to_plot.shape)
    print("Saving featues to plot ...")
    np.save('features.npy', features_to_plot)
    ...
    tsne = TSNE(perplexity = 25, n_iter = 3000, verbose=1)
